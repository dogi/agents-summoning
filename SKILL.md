...
| **Jules** (`google-labs-jules[bot]`) | Issue-driven Doer | `jules` label on an issue (reliable) / Jules app. On a PR it owns, only a comment that **mentions the bot** reaches the task — any surface there; the originating issue reaches nothing once the task has started. What comes back **replays the task commit** rather than editing it: an empty commit, or a revert of whatever you pushed. Re-file the issue to change the outcome; task chat in the web UI is the vendor's replanning channel. Mentions on foreign PRs go silent | own Jules-created PRs only — branch names have no dependable shape, so audit the `Co-authored-by` trailer, never the branch | `AGENTS.md` + per-repo memory (no `CLAUDE.md` support); configurable commit-authoring modes — audit via PR timeline + the `Co-authored-by: google-labs-jules[bot]` trailer it leaves; quotas 15/100/300 tasks/day by plan |
