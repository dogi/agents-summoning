...
6. **Audit by PR timeline, not `git log`.** Devin and Jules have configurable
   commit-authoring modes: their commits can wear the requesting user's git
   identity. The PR timeline is the only reliable attribution. OpenHands
