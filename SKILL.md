...
|---|---|---|---|---|
| **CodeRabbit** (`coderabbitai[bot]`) | Reviewer | auto on push (skips drafts & dependabot; auto-pauses after 5 reviewed commits) · `@coderabbitai review` / `resolve` / `approve` / `pause` · `help` lists the rest, including the Pro-gated fixers and generators. Answers in minutes; chats in threads | only opt-in (`fix ci commit`, `autofix`, generators) | teachable per-repo **learnings** from PR discussion; config via `.coderabbit.yaml` — can ingest `CLAUDE.md` as code guidelines (`knowledge_base.code_guidelines`) |
...
| **Devin** (`devin-ai-integration[bot]`) | Doer, instruction-following | `@devin <ask>` — session link ~10 s; one session **adopts** the PR, later mentions join it. Reviews: **`/devin review`** fires in seconds and tolerates trailing text, while `@devin review` goes silent with any extra text. Extra findings hide behind the web UI; a session can relay them | yes, unless leashed in the mention | Knowledge ingests `CLAUDE.md`/`AGENTS.md`; ⚠️ commit identity is a configurable "commit authoring mode" — audit via PR timeline, not `git log` |
...
| **Jules** (`google-labs-jules[bot]`) | Issue-driven Doer | `jules` label on an issue (reliable) / Jules app. On a PR it owns, only a comment that **mentions the bot** reaches the task — any surface there, nothing from the issue once started. The reply **replays the task commit**: an empty commit, or a revert of what you pushed. Re-file the issue to change the outcome; task chat is the vendor's replanning channel. Foreign-PR mentions go silent | own Jules-created PRs only — branch names have no dependable shape, so audit the `Co-authored-by` trailer, never the branch | `AGENTS.md` + per-repo memory (no `CLAUDE.md` support); configurable commit-authoring modes — audit via PR timeline + the `Co-authored-by: google-labs-jules[bot]` trailer it leaves; quotas 15/100/300 tasks/day by plan |
...
6. **Audit by PR timeline, not `git log` alone.** Devin and Jules have
   ...
   the trailers, but never take their absence as proof no agent was involved —
   and note that a **squash merge drops them**, leaving `main` with neither
   the trailer nor the branch commits. OpenHands
