...
| **Jules** (`google-labs-jules[bot]`) | Issue-driven Doer | `jules` label on an issue (reliable) / Jules app. PR feedback only on **Jules-created PRs**, and only through a comment that **mentions the bot** — any surface on the PR carries it, conversation or review thread; a mention on the originating issue after task start reaches nothing, and a comment without a mention reaches nothing anywhere (Reactive Mode; assume it is on, since the greeting describes a default mode you may not be in). What comes back is a **replay of the task commit** — same tree, same message: an empty commit where the branch already matches, a **revert of your work** where you have pushed to it. The plan is fixed at task start, so **re-file the issue rather than steering the PR**; task chat in the web UI is the documented replanning channel. Mentions on foreign PRs go silent (no session owns the branch) | own Jules-created PRs only — including follow-up commits there on accepted review feedback (branch names have no dependable shape — four summons produced three shapes — `jules-<slug>-<taskID>`, bare `<slug>-<taskID>` and `jules-<taskID>-<hash>`; audit on the co-author trailer, never the branch) | `AGENTS.md` + per-repo memory (no `CLAUDE.md` support); configurable commit-authoring modes — audit via PR timeline + the `Co-authored-by: google-labs-jules[bot]` trailer it leaves; quotas 15/100/300 tasks/day by plan |
...
6. **Audit by PR timeline, not `git log` alone.** Devin and Jules have
   ...
   and note that a **squash merge drops them**, leaving `main` with neither the
   trailer nor the branch commits (2026-08-23, `f4076db`). OpenHands
