...
| **Jules** | connect the repo in the [Jules app](https://jules.google). Mentions do **not** summon it on either surface — silent on a PR and on an issue in the same test (2026-08-21); the `jules` label on an issue is the only reliable summon, and it means "implement this issue". On a PR it **owns**, comments land — 👀, then follow-up commits | apply the `jules` label to an issue you actually want implemented → `google-labs-jules[bot]` acks on the issue within seconds, PR within minutes | silence |
